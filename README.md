...
Baumert–Hall array. The second list is the claim of a Hadamard matrix of order
4·515 = 2060. Cati and Pasechnik record the entry as unverified: missing details
left them unable to verify the construction. The gap can be made precise.
...
entry 1949(4), machine-diffed; re-checked at release date. A database records
what its authors knew, not what exists.
...
certificate rebuilds both matrices from the banked seeds, re-checks Theorem 3's
hypotheses on the actual matrices, hands each result to the trust chain, and
